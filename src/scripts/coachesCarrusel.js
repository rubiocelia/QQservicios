$(document).ready(function () {
  ...
  const textObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
          if (entry.isIntersecting) {
              entry.target.classList.add('animate__animated', 'animate__fadeInUp'); // Agrega clases de animación al texto
          } else {
              entry.target.classList.remove('animate__animated', 'animate__fadeInUp'); // Elimina clases de animación del texto
          }
      });
  });
